...
open class IconButton : UIButton{
    ...
    public init(icon: String, tintColor: UIColor = .label, backgroundColor: UIColor? = nil, withBorder: Bool = false){
    ...
    public init(image: String, tintColor: UIColor = .label, backgroundColor: UIColor? = nil, withBorder: Bool = true){
        self.hasBorder = withBorder
        super.init(frame: .zero)
    ...
    public func setIcon(icon: String, tintColor: UIColor = .label){
        setImage(UIImage(systemName: icon)?.withTintColor(tintColor, renderingMode: .alwaysOriginal), for: .normal)
    }
